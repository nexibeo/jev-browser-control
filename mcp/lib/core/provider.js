...
export function makeProvider(settings, { fetchImpl = fetch, settingsName = 'the Jev Browser Control settings' } = {}) {
  ...
  async function post(url, key, body, { signal } = {}) {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        // A dropped connection is usually a blip: retry for about 5 s before giving up.
        if (attempt < 3) {
          await new Promise((r) => setTimeout(r, 700 * 2 ** attempt));
          continue;
        }
        const cause = err.cause?.code || err.cause?.message;
        throw new ProviderError(`Could not reach ${new URL(url).host}: ${err.message}${cause ? ` (${cause})` : ''}. Check the internet connection.`, { code: 'network' });
      }
